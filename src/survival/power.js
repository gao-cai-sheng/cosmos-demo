...
export const freshPower=()=>({started:false,plan:null,site:null,installed:false,links:[],online:false,energy:48,hours:0});
export function normalizePower(raw){const s=freshPower();if(!raw?.started)return s;s.started=true;s.plan=SITES.some(p=>p.id===raw.plan)?raw.plan:null;s.site=raw.site===s.plan?s.plan:null;s.installed=!!s.site&&raw.installed===true;s.links=s.installed?['solar','bus'].filter(x=>(Array.isArray(raw.links)?raw.links:[]).includes(x)):[];s.online=s.links.length===2&&raw.online===true;s.energy=Number.isFinite(raw.energy)?Math.max(0,Math.min(60,raw.energy)):48;s.hours=Number.isFinite(raw.hours)?Math.max(0,raw.hours):0;return s;}
...
export function powerAction(raw,a,{eligible,outside,player,heightAt}){
 ...
 if(a.type==='install-power'&&outside&&near(site)&&s.site===s.plan&&survey(site,heightAt).ok)s.installed=true;
