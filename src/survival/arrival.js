...
import {SITES,equipment,survey,powerAction,tickPower,cableUsed} from './power.js';
...
export function mountArrival({scene,camera,canvas,heightAt}){
  ...
  function dispatch(action){capture();if(['start-power','plan-site','survey-site','install-power','connect-solar','connect-bus','commission-power'].includes(action.type)){state.power=powerAction(state.power,action,{eligible:complete(state),outside:state.environment==='outside',player:state.player,heightAt});powerWorld.rebuild(state.power);}else state=applyAction(state,action);save();render();}
  ...
  function render(){
    ...
    $('arrival-next').textContent=done?'第①步完成。打开“选址与接电”，开始部署营地电源。':!suitReady()?'先完成四项宇航服自检，再通过气闸出舱。':!state.refuge?'检查舱内储备终端，确认独立应急物资。':state.cargo.length<3?'出舱步行至三个货箱，逐一核对封签与清单。':!state.returned?'物资齐备。返回应急舱，验证最后一段退路。':'继续检查随船物资。';
    ...
    if(state.power.started){
      ...
      $('arrival-next').textContent=p.online?'第②步完成。下一步：部署压力帐篷并检漏入住。':!p.plan?'打开选址与接电，比较三个候选位置。':!p.site?'沿场景标记步行到候选地，现场确认地基。':!p.installed?'地基已确认，在现场展开随船电源套件。':!p.links.includes('solar')?'走近主电池，连接太阳能输入端口。':!p.links.includes('bus')?'走近配电箱，连接主电池输出端口。':'在配电箱处完成通电验收。';
      $('arrival-checklist').innerHTML=[['现场选址',!!p.site],['电源设备展开',p.installed],['端口连接',p.links.length===2],['通电验收',p.online]].map(([n,d],i)=>`<li class="${d?'done':''}"><span>${d?'✓':i+1}</span><div>${n}</div></li>`).join('');
    }
    $('power-readout').hidden=!state.power.started;
    $('power-readout').textContent=`主电池 ${state.power.energy.toFixed(2)} / 60 kWh · 电缆余量 ${100-cableUsed(state.power)} m`;
  ...
  function pause(){open('pause','任务已暂停',`<p>继续时从当前位置恢复。进度保存在此浏览器中。</p>`,[{label:'继续任务',primary:true,run:close},{label:'返回首页',run(){save();location.href='../index.html';}},{label:'重新开始生存开局',disabled:!!cycle,run(){open('reset','重置生存准备进度？','<p>这会清除本次物资检查、接电施工与电池进度。原有火星探索任务独立保留。</p>',[{label:'保留进度',primary:true,run:pause},{label:'确认重新开始',run(){state=freshState();restore();close();}}]);}}]);}
  function powerPanel(){
    const p=state.power;
    if(!p.started){open('power','把第一路电接起来。','<p>物资与退路已确认。接下来比较地基、现场展开一组太阳能阵列、主电池和配电箱，再逐一连接端口。</p><p class="fine">从已核对物资中部署：太阳能2套中使用1套、主电池1套、配电箱1套、电缆最多100 m。教学白昼发电4 kW，配电验收负载0.2 kW；暂不运行昼夜或生命资源消耗。</p>',[{label:'开始选址接电',primary:true,run(){dispatch({type:'start-power'});close();powerPanel();}}]);return;}
    const choices=SITES.map(site=>({site,check:survey(site,heightAt)}));
    open('power',p.online?'第②步完成 · 电源已投运':'营地选址与接电',`<p>候选地采用当前地形采样，最大坡度需不超过16°。选择只放置引导标记，须出舱到现场确认和安装。</p>${choices.map(({site,check})=>`<h3>${site.name} ${p.plan===site.id?'· 当前计划':''}</h3><p>距能源货箱 ${Math.round(Math.hypot(site.x+7,site.z-10))} m · 最大采样坡度 ${check.slope.toFixed(1)}° · ${check.ok?'地形合格':'坡度过大'}</p>`).join('')}${rows([['太阳能套件余量',p.installed?'1 / 2 套':'2 / 2 套'],['主电池',p.energy.toFixed(2)+' / 60 kWh'],['剩余电缆',(100-cableUsed(p))+' m'],['光伏输入',p.links.includes('solar')?'已连接 · 4 kW':'未连接 · 0 kW'],['配电输出',p.online?'已验收 · 0.2 kW 教学负载':'未投运']])}<p class="fine">功率与电量分别记账。模拟1小时对应现实120秒；满电停充，面板打开、暂停和后台不推进电量。下一步压力帐篷尚未施工。</p>`,[...(!p.installed?choices.map(({site,check})=>({label:'选择 '+site.name,disabled:!check.ok,run(){dispatch({type:'plan-site',id:site.id});close();}})):[]),{label:'返回现场',primary:true,run:close}]);
  }
  function powerTarget(){const p=state.power;if(!p.started||!p.plan||state.environment!=='outside')return null;const site=SITES.find(x=>x.id===p.plan),eq=equipment(p.plan);return !p.site?{...site,id:'power-device',action:'survey-site',name:'现场确认 '+site.name}:!p.installed?{...site,id:'power-device',action:'install-power',name:'展开电源套件'}:!p.links.includes('solar')?{...eq.battery,id:'power-device',action:'connect-solar',name:'连接光伏输入 · 使用12 m电缆'}:!p.links.includes('bus')?{...eq.bus,id:'power-device',action:'connect-bus',name:'连接配电箱 · 使用6 m电缆'}:{...eq.bus,id:'power-device',action:p.online?'status':'commission-power',name:p.online?'查看营地电源':'通电验收'};}
  function nearest(){const p=astronaut.position;const targets=state.environment==='inside'?[{id:'refuge',name:'检查应急储备',...REFUGE},{id:'door',name:suitReady()?'通过气闸出舱':'出舱前需完成宇航服自检',x:0,z:0}]:[{id:'door',name:'通过气闸返回应急舱',x:0,z:6},...CARGO.map(c=>({...c,name:'核对 '+c.name}))];const power=powerTarget();if(power)targets.push(power);return targets.map(t=>({...t,distance:Math.hypot(p.x-t.x,p.z-t.z)})).sort((a,b)=>a.distance-b.distance)[0];}
  function interact(){if(modal||cycle)return;capture();const t=nearest();if(t.distance>(t.id==='door'?2.8:3))return;if(t.id==='door'){
    ...
    }else if(t.id==='power-device'){if(t.action==='status')powerPanel();else dispatch({type:t.action});}else if(t.id==='refuge')refuge();else cargo(CARGO.find(c=>c.id===t.id));}
  $('arrival-power').onclick=powerPanel;
  ...
  function update(dt){
    ...
    if(!modal&&!cycle){state.power=tickPower(state.power,dt);$('power-readout').textContent=`主电池 ${state.power.energy.toFixed(2)} / 60 kWh · 电缆余量 ${100-cableUsed(state.power)} m`;}
    ...
    for(const {t,el} of labels){let x=t.x,z=t.z;if(t.id==='door'&&state.environment==='inside')z=0;const dist=Math.hypot(astronaut.position.x-x,astronaut.position.z-z);projected.set(x,world.floor(x,z)+(t.id==='door'?2.3:1.8),z).project(camera);const available=state.environment==='inside'?(t.id==='door'||t.id==='refuge'):t.id!=='refuge';el.hidden=dist<2||!available||!!modal||!!cycle||projected.z< -1||projected.z>1||Math.abs(projected.x)>.94||Math.abs(projected.y)>.8;if(!el.hidden){const sx=(projected.x*.5+.5)*innerWidth,sy=(-projected.y*.5+.5)*innerHeight;el.hidden=[...ui.querySelectorAll('.arrival-objective,.arrival-suit')].some(panel=>{const b=panel.getBoundingClientRect();return sx>b.left-80&&sx<b.right+80&&sy>b.top-15&&sy<b.bottom+15;});}el.style.left=(projected.x*.5+.5)*innerWidth+'px';el.style.top=(-projected.y*.5+.5)*innerHeight+'px';el.textContent=`${t.code?t.code+' / ':''}${t.name} · ${Math.round(dist)} m${state.cargo.includes(t.id)?' ✓':''}`;}
  ...
  const baseUpdate=update;function updateWithPower(dt){baseUpdate(dt);const t=powerTarget();powerMarker.hidden=!t||!!modal||!!cycle;if(t){projected.set(t.x,heightAt(t.x,t.z)+2,t.z).project(camera);powerMarker.hidden ||= projected.z< -1||projected.z>1||Math.abs(projected.x)>.95||Math.abs(projected.y)>.8;powerMarker.style.left=(projected.x*.5+.5)*innerWidth+'px';powerMarker.style.top=(-projected.y*.5+.5)*innerHeight+'px';powerMarker.textContent=`${t.name} · ${Math.round(Math.hypot(astronaut.position.x-t.x,astronaut.position.z-t.z))} m`;}}
  save();render();
  const api={astronaut,walking:true,active:false,update:updateWithPower,get state(){return structuredClone(state);},get cycling(){return !!cycle;},get paused(){return !!modal;},setPaused(value){if(value)pause();else if(modal)close();}};window.__SURVIVAL=api;return api;
}
